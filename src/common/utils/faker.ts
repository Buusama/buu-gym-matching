...
const fakers = {
    ...
    fakePhotos(): Photo[] {
        const photos: Photo[] = []
        for (let i = 0; i < 15; i++) {
            photos.push({ fileName: 'profile-' + _.random(1, 15) + '.jpg', type: 'Image', size: '1.2 MB' })
        }
    ...
    fakeDates(): Date[] {
        const dates: Date[] = []
        for (let i = 0; i < 5; i++) {
            dates.push({ date: dayjs.unix(_.random(1586584776897, 1672333200000) / 1000).format('DD MMMM YYYY') })
        }
    ...
    fakeTotals(): Total[] {
        return _.shuffle([{ total: _.random(20, 220) }, { total: _.random(20, 120) }, { total: _.random(20, 50) }])
    },
    fakeStocks(): Stock[] {
        return _.shuffle([{ stock: _.random(50, 220) }, { stock: _.random(50, 120) }, { stock: _.random(50, 50) }])
    },
